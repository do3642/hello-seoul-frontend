...
function SpotsAllTourist() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8888/api/tourist-spots?lang=${replaceLang}&page=${page}&size=8`);      
      // 데이터가 있으면 추가, 없으면 hasMore를 false로 설정
      if (response.data.length === 0) {
        setHasMore(false);
      } else {
        setTouristSpots(prevSpots => [...prevSpots, ...response.data]);
